...
select has_function(
  ...
  array['uuid','uuid','date','time without time zone','time without time zone','numeric','text'],
  'Rezervasyon fiyat teklif RPCsi vardır'
);
select ok(
  pg_get_functiondef('public.create_marketplace_reservation(uuid,uuid,date,time,time,numeric,text)'::regprocedure)
    ilike '%P0002%',

...
export function AdminVenueQueue() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900">Onay Kuyruğu</h1>
      ...
      <div className="mt-5 space-y-3">
        ...
        {venues?.map((venue) => (
          <div
            key={venue.id}
            className="flex flex-col gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-soft"
          >
            <div className="flex flex-col gap-4 sm:flex-row">
              ...
              <div className="min-w-0 flex-1">
                ...
                {venue.description && (
                  <p className="mt-2 line-clamp-3 text-sm text-slate-600">{venue.description}</p>
                )}
                {venue.amenities.length > 0 && (
                  <p className="mt-2 text-xs text-slate-400">{venue.amenities.join(' · ')}</p>
                )}
              </div>
            </div>

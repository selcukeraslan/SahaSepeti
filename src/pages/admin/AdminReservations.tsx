...
export function AdminReservations() {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-slate-500 dark:text-ink-400">Platformdaki tüm rezervasyonlar (salt okunur).</p>
      ...
      <div className="mt-4 grid max-w-lg gap-3 sm:grid-cols-2">
        ...
        <input
          ...
          className="h-11 rounded-xl border border-slate-300 bg-white px-3.5 text-sm text-slate-900 dark:border-ink-700 dark:bg-ink-900 dark:text-ink-50"
        />
      </div>
      ...
      <div className="mt-5 space-y-2">
        ...
        {reservations?.map((reservation) => (
          <div
            key={reservation.id}
            className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 dark:border-ink-800 dark:bg-ink-900"
          >

...
export function ForgotPassword() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast('Şifre yenileme bağlantısı e-posta adresinize gönderildi.', 'success')
  ...
  return (
    <>
      <Seo title="Şifremi Unuttum" canonicalPath="/sifremi-unuttum" />
      <AuthShell eyebrow="Hesabını kurtar" title="Şifremi unuttum" description="Şifre yenileme bağlantısını e-posta adresine gönderelim." footer={<Link to="/giris" className="font-semibold text-primary-600 hover:text-primary-700">Giriş ekranına dön</Link>}>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
          ...
          <Button type="submit" size="lg" isLoading={isSubmitting} className="mt-2 rounded-full">Bağlantı Gönder</Button>
        </form>
      </AuthShell>
    </>
  )
